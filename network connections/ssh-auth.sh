...
ssh -i ~/.ssh/bash_example_rsa -p 2022 grayroom@localhost

echo && echo "Подключаемся к удаленной машине через ssh и выполняем только одну команду"
ssh -i ~/.ssh/bash_example_rsa -p 2022 grayroom@localhost "ls -a"

echo "Подключаемся к удаленной машине через ssh в режиме дебага"
ssh -v -i ~/.ssh/bash_example_rsa -p 2022 grayroom@localhost